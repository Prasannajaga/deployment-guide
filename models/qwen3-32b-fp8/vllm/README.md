...
```bash
kubectl get pvc -n dynamo-bench model-cache compilation-cache perf-cache
```

Select one recipe and run it until its benchmark finishes. It takes about 1H~2H per experiment:
