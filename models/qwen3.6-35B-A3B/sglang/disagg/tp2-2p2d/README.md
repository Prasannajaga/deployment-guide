# SGLang disaggregated tp2-2p2d

This recipe runs 2 TP=2 prefill workers and 2 TP=2 decode workers.
...
kubectl logs -n "$NAMESPACE" -l "$PREFLIGHT_GRAPH_LABEL" \
  ...
  tee -a "/tmp/qwen36-sglang-tp2-2p2d-preflight.log"
...
uses four GPUs and must be deleted before the full eight-GPU recipe. A pass
...
kubectl logs -n "$NAMESPACE" -l "$GRAPH_LABEL" \
  --all-containers --prefix --tail=1500 |
  tee "/tmp/qwen36-sglang-tp2-2p2d-startup.log"

if grep -Ei 'traceback|assertionerror|notimplementederror|nixl_err|out of memory|does not support|unsupported' \
  "/tmp/qwen36-sglang-tp2-2p2d-startup.log"; then
  echo "SGLang startup compatibility gate failed" >&2
  exit 1
fi

grep -Ei 'sglang|nixl|ucx|rdma|prefill|decode|transfer|ready' \
  "/tmp/qwen36-sglang-tp2-2p2d-startup.log" | tail -300
```
...
kind: Job
metadata:
  name: qwen36-sglang-tp2-2p2d-perf
spec:
  backoffLimit: 0
  completions: 1
  parallelism: 1
  ...
  template:
    metadata:
      labels:
        app: qwen36-sglang-tp2-2p2d-perf
    spec:
      restartPolicy: Never
      tolerations:
        ...
        - name: perf
          ...
          env:
            ...
            - name: TOKENIZER
              value: /opt/models/hub/models--Qwen--Qwen3.6-35B-A3B-FP8/snapshots/95a723d08a9490559dae23d0cff1d9466213d989
            - name: ENDPOINT
              value: q36-sgl-pd-tp2-2p2d-frontend:8000
            - name: ISL
              value: "8000"
            - name: OSL
              value: "1024"
