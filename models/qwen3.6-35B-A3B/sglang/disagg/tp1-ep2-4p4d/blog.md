...
The **Decode Worker** is dedicated exclusively to token-by-token autoregressive generation, pulling live KV and recurrent Mamba states directly from prefill workers via NIXL RDMA and streaming generated tokens back to the Dynamo frontend.
...
- **Sub-20ms Transfer Latency:** After initial warmup, the **NIXL Average Transfer Time** settles into a tight **15 ms to 20 ms** window per request. Moving massive 64K-token KV caches and Mamba recurrent states across nodes in under 20 ms ensures decode workers fire autoregressive kernels almost instantly upon prefill completion.
